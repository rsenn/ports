...
LIBRARIES = $(boost_LIBRARIES:%=libboost_%$(MT).la)
TARGETS = $(LIBRARIES)
...
ifneq ($(LIBRARIES),)
	...
	$(LIBTOOL) --tag=CXX --mode=finish $(DESTDIR)$(libdir)
endif
...
# ---------------------------------------------------------------------------
CPPFLAGS = -isystem .
CPPFLAGS += -DBOOST_NO_STD_WSTREAMBUF -UBOOST_NO_MT -DBOOST_MT -DBOOST_HAS_THREADS


libboost_date_time$(MT).la: \
  libs/date_time/src/gregorian/date_generators$(MT).lo \
  libs/date_time/src/gregorian/greg_month$(MT).lo \
  libs/date_time/src/gregorian/greg_weekday$(MT).lo

libboost_filesystem$(MT).la: \
  $(patsubst %.cpp,%$(MT).lo,$(wildcard libs/filesystem/src/*.cpp))

libboost_iostreams$(MT).la: LIBADD = -lbz2 -lz
libboost_iostreams$(MT).la: \
  libs/iostreams/src/bzip2$(MT).lo \
  libs/iostreams/src/file_descriptor$(MT).lo \
  libs/iostreams/src/mapped_file$(MT).lo \
  libs/iostreams/src/zlib$(MT).lo

libboost_program_options$(MT).la: \
  libs/program_options/src/cmdline$(MT).lo \
  libs/program_options/src/config_file$(MT).lo \
  libs/program_options/src/convert$(MT).lo \
  libs/program_options/src/options_description$(MT).lo \
  libs/program_options/src/parsers$(MT).lo \
  libs/program_options/src/positional_options$(MT).lo \
  libs/program_options/src/utf8_codecvt_facet$(MT).lo \
  libs/program_options/src/value_semantic$(MT).lo \
  libs/program_options/src/variables_map$(MT).lo \
  libs/program_options/src/winmain$(MT).lo

libboost_python$(MT).la: \
  libs/python/src/converter/arg_to_python_base$(MT).lo \
  libs/python/src/converter/builtin_converters$(MT).lo \
  libs/python/src/converter/from_python$(MT).lo \
  libs/python/src/converter/registry$(MT).lo \
  libs/python/src/converter/type_id$(MT).lo \
  libs/python/src/dict$(MT).lo \
  libs/python/src/errors$(MT).lo \
  libs/python/src/list$(MT).lo \
  libs/python/src/long$(MT).lo \
  libs/python/src/module$(MT).lo \
  libs/python/src/numeric$(MT).lo \
  libs/python/src/object/class$(MT).lo \
  libs/python/src/object/enum$(MT).lo \
  libs/python/src/object/function$(MT).lo \
  libs/python/src/object/inheritance$(MT).lo \
  libs/python/src/object/iterator$(MT).lo \
  libs/python/src/object/life_support$(MT).lo \
  libs/python/src/object/pickle_support$(MT).lo \
  libs/python/src/object_operators$(MT).lo \
  libs/python/src/object_protocol$(MT).lo \
  libs/python/src/slice$(MT).lo \
  libs/python/src/str$(MT).lo \
  libs/python/src/tuple$(MT).lo \
  libs/python/src/wrapper$(MT).lo

libboost_regex$(MT).la: \
  $(patsubst %.cpp,%$(MT).lo,$(wildcard libs/regex/src/*.cpp))

libboost_serialization$(MT).la: \
  libs/serialization/src/basic_archive$(MT).lo \
  libs/serialization/src/basic_archive_impl$(MT).lo \
  libs/serialization/src/basic_iarchive$(MT).lo \
  libs/serialization/src/basic_iserializer$(MT).lo \
  libs/serialization/src/basic_oarchive$(MT).lo \
  libs/serialization/src/basic_oserializer$(MT).lo \
  libs/serialization/src/basic_pointer_iserializer$(MT).lo \
  libs/serialization/src/basic_pointer_oserializer$(MT).lo \
  libs/serialization/src/basic_serializer_map$(MT).lo \
  libs/serialization/src/basic_text_iprimitive$(MT).lo \
  libs/serialization/src/basic_text_oprimitive$(MT).lo \
  libs/serialization/src/basic_xml_archive$(MT).lo \
  libs/serialization/src/binary_iarchive$(MT).lo \
  libs/serialization/src/binary_oarchive$(MT).lo \
  libs/serialization/src/extended_type_info$(MT).lo \
  libs/serialization/src/extended_type_info_no_rtti$(MT).lo \
  libs/serialization/src/extended_type_info_typeid$(MT).lo \
  libs/serialization/src/polymorphic_iarchive$(MT).lo \
  libs/serialization/src/polymorphic_oarchive$(MT).lo \
  libs/serialization/src/text_iarchive$(MT).lo \
  libs/serialization/src/text_oarchive$(MT).lo \
  libs/serialization/src/void_cast$(MT).lo \
  libs/serialization/src/xml_grammar$(MT).lo \
  libs/serialization/src/xml_iarchive$(MT).lo \
  libs/serialization/src/xml_oarchive$(MT).lo

libboost_signals$(MT).la: \
  $(patsubst %.cpp,%$(MT).lo,$(wildcard libs/signals/src/*.cpp))
  
libboost_text_exec_monitor$(MT).la: \
  libs/test/src/compiler_log_formatter$(MT).lo \
  libs/test/src/execution_monitor$(MT).lo \
  libs/test/src/framework$(MT).lo \
  libs/test/src/plain_report_formatter$(MT).lo \
  libs/test/src/progress_monitor$(MT).lo \
  libs/test/src/results_collector$(MT).lo \
  libs/test/src/results_reporter$(MT).lo \
  libs/test/src/test_main$(MT).lo \
  libs/test/src/test_tools$(MT).lo \
  libs/test/src/unit_test_log$(MT).lo \
  libs/test/src/unit_test_main$(MT).lo \
  libs/test/src/unit_test_monitor$(MT).lo \
  libs/test/src/unit_test_parameters$(MT).lo \
  libs/test/src/unit_test_suite$(MT).lo \
  libs/test/src/xml_log_formatter$(MT).lo \
  libs/test/src/xml_report_formatter$(MT).lo

libboost_unit_test_framework$(MT).la: \
  libs/test/src/compiler_log_formatter$(MT).lo \
  libs/test/src/execution_monitor$(MT).lo \
  libs/test/src/framework$(MT).lo \
  libs/test/src/plain_report_formatter$(MT).lo \
...
libboost_wave$(MT).la: \
  $(patsubst %.cpp,%$(MT).lo,$(wildcard libs/wave/src/*.cpp libs/wave/src/cpplexer/re2clex/*.cpp))

libboost_wserialization$(MT).la: \
  libs/serialization/src/binary_wiarchive$(MT).lo \
  libs/serialization/src/binary_woarchive$(MT).lo \
  libs/serialization/src/basic_text_wiprimitive$(MT).lo \
  libs/serialization/src/basic_text_woprimitive$(MT).lo \
  libs/serialization/src/codecvt_null$(MT).lo \
  libs/serialization/src/text_wiarchive$(MT).lo \
  libs/serialization/src/text_woarchive$(MT).lo \
  libs/serialization/src/utf8_codecvt_facet$(MT).lo \
  libs/serialization/src/xml_wgrammar$(MT).lo \
  libs/serialization/src/xml_wiarchive$(MT).lo \
  libs/serialization/src/xml_woarchive$(MT).lo

libboost_thread$(MT).la: \
  libs/thread/src/barrier$(MT).lo \
  libs/thread/src/condition$(MT).lo \
  libs/thread/src/exceptions$(MT).lo \
  libs/thread/src/mutex$(MT).lo \
  libs/thread/src/once$(MT).lo \
  libs/thread/src/recursive_mutex$(MT).lo \
  libs/thread/src/thread$(MT).lo \
  libs/thread/src/tss$(MT).lo \
  libs/thread/src/tss_dll$(MT).lo \
  libs/thread/src/tss_hooks$(MT).lo \
  libs/thread/src/tss_null$(MT).lo \
  libs/thread/src/tss_pe$(MT).lo \
  libs/thread/src/xtime$(MT).lo

libboost_thread_mac$(MT).la: \
  libs/thread/src/mac/delivery_man$(MT).lo \
  libs/thread/src/mac/dt_scheduler$(MT).lo \
  libs/thread/src/mac/execution_context$(MT).lo \
  libs/thread/src/mac/init$(MT).lo \
  libs/thread/src/mac/msl_replacements/assert$(MT).lo \
  libs/thread/src/mac/msl_replacements/console_io$(MT).lo \
  libs/thread/src/mac/msl_replacements/malloc$(MT).lo \
  libs/thread/src/mac/msl_replacements/news_and_deletes$(MT).lo \
  libs/thread/src/mac/msl_replacements/time$(MT).lo \
  libs/thread/src/mac/os$(MT).lo \
  libs/thread/src/mac/ot_context$(MT).lo \
  libs/thread/src/mac/remote_call_manager$(MT).lo \
  libs/thread/src/mac/safe$(MT).lo \
  libs/thread/src/mac/scoped_critical_region$(MT).lo \
  libs/thread/src/mac/st_scheduler$(MT).lo \
  libs/thread/src/mac/thread_cleanup$(MT).lo
