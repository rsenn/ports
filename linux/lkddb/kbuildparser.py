...
import sys, os, os.path, re, lkddb
...
def parse_kbuild(kerneldir, subdir, deps=None):
	...
	lkddb.log("I don't know the directory %s" % subdir)
	return
	...
	if not os.path.isfile(mk2):
	    lkddb.log("parse_kbuild: could not find included file (from %s): %s" % (mk, mk2))
	    return
	f = open(mk2)
	src += " " + kbuild_normalize.sub(" ", f.read())
	f.close()
	...
	# rule-$(dep): file.o
		...
		v = d.copy()
		    ...
		    v.update(dependencies[fc])
                ...
                lkddb.log("parse_kbuild: unknow 'make target' in %s: '%s'" % (subdir, f))
