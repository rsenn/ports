#!/usr/bin/python
#:  lkddb.py : generic support for build-drivers-db.py module
#
...
def log_init(verbose=False, logfile=sys.stdout):
...
def lkddb_print(output_filename, header):
    ...
    for scanner, res, dep, filename in devices:
        dep = " ".join(dep)
	#log("# Checking device: %s, %s, %s # %s" %
	    #(table.name, res, dep, filename))
...
class scanner_array_of_struct(scanner):
	...
	regex = r"\b%s\s+\w+\[\d*\]\s*\w*\s*=\s*\{(.*?)\}\w*;" % struct_name
...
class scanner_struct(scanner):
    ...
    def __init__(self, name, struct_name):
        regex = r"\b%s\s+\w+\s*\w*\s*=\s*(\{.*?\})\w*;" % struct_name
        scanner.__init__(self, name, regex)
	self.splitter = split_structs
...
def strings(field, dictionary, default='""'):
	...
	if v[0] == '{'  and  v[-1] == '}':
		...
		return strings("recurse", {"recurse": value}, default)
